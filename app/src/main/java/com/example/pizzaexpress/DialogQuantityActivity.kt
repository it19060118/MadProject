...
class DialogQuantityActivity : AppCompatActivity() {
    var quantityTv: TextView? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_dialog_quantity)
        //init layout views
        val decrementBtn = findViewById<ImageButton>(R.id.decrementBtn)
        quantityTv = findViewById(R.id.quantityTv)
        val incrementBtn = findViewById<ImageButton>(R.id.incrementBtn)
        val continueBtn = findViewById<Button>(R.id.continueBtn)
        val originalPriceTv = findViewById<TextView>(R.id.originalPriceTV)


        //back button redirect to the valueOffer page
        val backImgBtn = findViewById<ImageView>(R.id.floatingActionButton14)
        backImgBtn.setOnClickListener {
            val launchActivity1 = Intent(applicationContext, Displayed_Option_list::class.java)
    ...
    fun increaseItemQty(view: View?) {
        var temp: Int
        val text: String
        temp = quantityTv!!.text.toString().toInt()
        temp++
        text = temp.toString()
        quantityTv!!.text = text
    }
